...
ProtoSpline3::ProtoSpline3(const std::vector<ProtoVector3>& controlPts, int interpDetail, bool isCurveClosed, float smoothness) :
...
void ProtoSpline3::init() {
    ...
    for (int i = 0; i < controlPts.size() - 3; i++) {
        ...
        for (float t = 0; t < 1; t += step) {
            ...
            ProtoVector3 v = smoothness * ((2 * v1) +
                    (-v0 + v2) * t +
...
void ProtoSpline3::parallelTransport() {
    ...
    //frenetFrames.push_back(FrenetFrame(verts.at(0), ProtoVector3(1,0,0), ProtoVector3(0,-1,0), ProtoVector3(0,0,-1))); // add first vert
    // std::cout << "in createFrenetFrame():  verts.size() = " << verts.size() << std::endl;
    std::vector<ProtoVector3> tans;
    float theta = 0.0;
    ProtoVector3 cp0, cp1, cp2;
    ProtoVector3 tan, biNorm, norm, nextBiNorm, nextNorm;
    ...
    for (int i = 0; i < verts.size(); i++) {
        ...
        if (i == 1) {
            ...
            if (biNorm.mag() == 0) {
                 ...
                 if (cp1.x !=0 && cp2.x !=0){
                     biNorm = ProtoVector3(0, 1, 0);
                 }
                 if (cp1.y !=0 && cp2.y !=0){
                     biNorm = ProtoVector3(0, 0, -1);
                 }  
                 if (cp1.z !=0 && cp2.z !=0){
                     biNorm = ProtoVector3(1, 0, 0);
                 } 
    ...
    for (int i = 0; i < tans.size() - 1; i++) {
        ...
        if (biNorm.mag() == 0) {
            nextNorm = norm;
            //frenetFrames.push_back(FrenetFrame(verts.at(i), ProtoVector3(1,1,1), ProtoVector3(1,1,1), ProtoVector3(1,1,1)));
            // std::cout << "norm = " << norm << std::endl;
        } else {
            theta = acos(tans.at(i).dot(tans.at(i + 1)));
            ProtoVector3 axis = tans.at(i);
